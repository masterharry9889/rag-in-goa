...
export default function STTProviderPage() {
  ...
  const fetchConfig = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setKeys({
          SARVAM_API_KEY: data.SARVAM_API_KEY || '',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage('STT Provider keys saved successfully!');
  ...
  return (
    <main className="min-h-screen bg-[#0f1115] text-gray-100 flex flex-col pt-32 pb-32 relative overflow-hidden selection:bg-brand-main/30">
      ...
      <motion.div 
        ...
        animate={{
          x: (mousePos.x - window.innerWidth/2) * 0.5,
          y: (mousePos.y - window.innerHeight/2) * 0.5,
        }}
      ...
      <section className="flex-1 px-6 relative z-10">
        <div className="max-w-3xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-400 text-lg font-light">Configure your API keys for STT services like Sarvam and ElevenLabs.</p>
          </motion.div>
          ...
          >
                ...
                >
                  ...
                  <div className="flex gap-3">
                    <input
                      ...
                      className="flex-1 bg-[#0f1115] border border-[#272b36] rounded-xl px-5 py-4 text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-brand-accent focus:border-transparent transition-all shadow-inner"
            ...
            {message && (
              <motion.div 
                ...
                animate={{ opacity: 1, y: 0 }}
                className={`mt-8 p-5 rounded-xl text-sm font-medium backdrop-blur-md ${message.includes('success') ? 'bg-brand-dark/20 text-brand-light border border-brand-main/30' : 'bg-red-500/20 text-red-300 border border-red-500/30'}`}
              >
                {message}
              </motion.div>
            )}
